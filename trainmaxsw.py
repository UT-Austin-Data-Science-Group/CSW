...
def main():
    ...
    if(args.slice_type=='sw'):
        if args.dataset.lower() == 'stl10':
            slicer = Base_Slicer(d=4608,L=1).cuda()
        else:
            slicer = Base_Slicer(d=8192,L=1).cuda()
        ...
        if args.dataset.lower() == 'stl10':
            slicer = ConvSlicer(L=1, ch=args.df_dim, bottom_width=6,type=args.slice_type)
        else:
            slicer = ConvSlicer(L=1, ch=args.df_dim, bottom_width=8,type=args.slice_type)
