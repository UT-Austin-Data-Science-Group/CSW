...
def main():
    ...
    if(args.slice_type=='prw'):
        if args.dataset.lower() == 'stl10':
            slicer = PRW_Slicer(d=4608,k=args.k).cuda()
        else:
            slicer = PRW_Slicer(d=8192,k=args.k).cuda()
    # elif (args.slice_type == 'gsw'):
    #     if (args.s_act == 'sigmoid'):
    #         s_act = nn.Sigmoid()
    #     if args.dataset.lower() == 'stl10':
    #         slicer = NonLinearBase_Slicer(d=4608, L=1,activation=s_act).cuda()
    #     else:
    #         slicer = NonLinearBase_Slicer(d=8192, L=1,activation=s_act).cuda()
    # elif (args.slice_type == 'ncsw' or args.slice_type == 'ncswd' or args.slice_type == 'ncsws'):
    #     if(args.s_act=='sigmoid'):
    #         s_act=nn.Sigmoid()
    #     if args.dataset.lower() == 'stl10':
    #         slicer = NonLinearConvSlicer(L=1, ch=args.df_dim, bottom_width=6,type=args.slice_type,activation=s_act)
    #     else:
    #         slicer = NonLinearConvSlicer(L=1, ch=args.df_dim, bottom_width=8,type=args.slice_type,activation=s_act)
    else:
        if args.dataset.lower() == 'stl10':
            slicer = PRWConvSlicer(k=args.k, ch=args.df_dim, bottom_width=6,type=args.slice_type)
        else:
            slicer = PRWConvSlicer(k=args.k, ch=args.df_dim, bottom_width=8,type=args.slice_type)
